...
export const ChatSidebar: React.FC<ChatSidebarProps> = ({ 
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed top-0 right-0 h-full w-full md:w-96 bg-white dark:bg-gray-900 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col
        ${isOpen ? 'translate-x-0' : 'translate-x-full'}
      `}>
        {/* Header */}
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-history-50 dark:bg-gray-800">
          <div className="flex items-center gap-2">
            <div className="bg-indigo-100 dark:bg-indigo-900/50 p-2 rounded-lg text-indigo-700 dark:text-indigo-300">
               <Bot size={20} />
            </div>
            <div>
              <h3 className="font-bold text-gray-800 dark:text-gray-100">History Assistant</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[180px]">{context}</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
             <button onClick={handleClear} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors" title="Clear Chat">
              <Trash2 size={18} />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50 dark:bg-black/20">
          {messages.map((msg) => (
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-gray-800 dark:bg-gray-700 text-white' : 'bg-indigo-600 text-white'}`}>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
          ...
          >
            <input
              ...
              className="flex-1 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 border-0 rounded-full px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
            />
            <button 
              type="submit" 
